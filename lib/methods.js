...
module.exports = {
  ...
  next: function (){
    ...
    // If yes, abort. If no, continue
    if(this.f_tries.wholeList > this.f_max_tries.wholeList &&
       this.f_tries.wholeList !== '?')
        return this.f_abort('Max all tries exceeded',
          new Error('this.f_tries.wholeList > this.f_max_tries.wholeList'));
    ...
    if(this.f_i === 0 && !this.f_retryMethodCall && this.f_to_log.has('start'))
      log('Starting task '
        ...
        + (this.f_max_tries.wholeList !== '?'
          ? '/' + RED(this.f_max_tries.wholeList) : '')
      );
