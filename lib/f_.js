...
var METHODS = {
	...
	next: function next(){
		...
		if(this.f_i === 0 && this.f_toLog.has('start'))
			...
			+ ' of max ' + RED(this.f_maxRetries.all+ 1));
...
/*
			// If a maxRetries is set for a certain method
			if(this.f_maxRetries[methodName] || this.f_maxRetries[methodName] === 0){

				// If the max tries for a certain method is exceeded abort
				if(this.f_retries[methodName] > this.f_maxRetries[methodName])
					return this.f_abort('`' + methodName + '` maxRetries exceeded',
						'this.f_retries[methodName] > this.f_maxRetries[methodName]');
		
				// If we're not at the max yet, just incr tries and carry on
				this.f_retries[methodName] += 1
			}
*/

		//console.log(this.f_retries[methodName]);

			// Logs method name. If maxRetries is set for a method, log about attempts
			if(this.f_toLog.has('next')) log('next: ' + BLUE(methodName) + (
				this.f_maxRetries[methodName] || this.f_maxRetries[methodName] === 0
					? '\tAttempt ' + GREEN(this.f_retries[methodName]) + ' of max '
						+ RED( this.f_maxRetries[methodName] + 1)
					: ''
			));
	...
	retryAll: function retryAll(desc, err){
		...
		if(this.onRetry) this.onRetry({
			...
			retries: this.f_retries.all,
			maxRetries: this.f_maxRetries.all
		});
	 ...
	 * Will increment maxRetries.methodName
	...
	retryThis: function retryThis(desc, err){
		...
		if(this.onRetry) this.onRetry({
			type: 'retryThis',
			calledFrom: this.f_functionFlow[this.f_i - 1],
			retries: this.f_retries[this.f_functionFlow[this.f_i - 1]],
			maxRetries: this.f_maxRetries[this.f_functionFlow[this.f_i - 1]]
		});
 	
		// Use this cuz f_ will make use of 'next' logic (maxRetries)
		this.f_i -= 1;

		// maxRetries logic should be HERE!!!!
...
function augment(toAugment, config){
	...
	if(!props.maxRetries.all && props.maxRetries.all !== 0)
		props.maxRetries.all = 9;


	props.functionFlow.forEach(function (methodName){
		if(!props.maxRetries[methodName])
			props.maxRetries[methodName] = 9;
	});
...
function setup(instance){
	...
	for(var method in instance.f_maxRetries)
