...
var METHODS = {
	...
	next: function next(){
		...
		if(this.f_i === 0 && this.f_toLog.has('start')) log('Starting task '
	...
	retryAll: function retryAll(desc, err){
		...
		if(this.f_toLog.has('retry')) log(
      RED('Retry') + (desc ? '  desc: ' + RED(desc) : '')
				+ (err  ? '  err: '  + RED(err)  : '') );
	...
	retryFrom: function retryFrom(method, desc, err){
		...
		if(methodIndex === -1){
			var logDesc = '@retryFrom, could not find method `' + method + '`.';
			this.f_addErr(logDesc, 'this.methodIndex === -1');

			if(this.f_toLog.has('retry'))
				logRed(logDesc + WHITE(' Will carry on normaly'));
		}
		// Set f_i to methodIndex, so `next` will carry on from there
		else this.f_i = methodIndex;
