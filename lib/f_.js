...
var METHODS = {
	...
	next: function next(){
		...
		if(this[methodName] && typeof this[methodName] === 'function'){
			...
			if(this.f_maxTries[methodName] || this.f_maxTries[methodName] === 0){
			...
			if(this.f_toLog.has('next')) log('next: ' + BLUE(methodName) + (
				this.f_maxTries[methodName] || this.f_maxTries[methodName] === 0
	...
	retryThis: function retryThis(desc, err){
		...
		if(this.onRetry) this.onRetry({
			...
			nextTry: this.f_tries[this.f_functionFlow[this.f_i]] + 1,
...
function augment(toAugment, config){
	...
	props.functionFlow.forEach(function (methodName){
		if(!props.maxTries[methodName] && props.maxTries[methodName] !== 0)
