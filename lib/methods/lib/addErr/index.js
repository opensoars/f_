...
module.exports = function f_addErr(message) {
  ...
  var current_flow = this.f_function_flow[this.f_flow_i-1];

  var err_object = {
    time: new Date().getTime(),
    error: new Error(message),
    current_flow_name: current_flow ? current_flow.name : '?'
  };
   ...
   * @proptery {string} current_flow - Which function f_log was called from
   */
